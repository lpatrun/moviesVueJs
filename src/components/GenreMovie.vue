<template>
  <div>
    <div class="mySingle"  v-for="(movie, index) in genreMovie" :key="index">
...
export default {
  created () {
    this.$store.dispatch('returnOneMovie', this.$route.params.id)
  },
  beforeDestroy () {
    this.$store.commit('clearGenre')
  },
  computed: {
    genreMovie () {
      return this.$store.state.oneGenreMovie
    }
  },
  components: {
